...
export default function NfseList() {
    ...
    // Apply all filters
    // ⚡ Bolt Optimization: Memoized the heavy group filtering/mapping logic.
    // Why: Prevents full O(N) recalculations on every render when expanding/collapsing accordion rows.
    // Impact: Eliminates UI lag on local state toggles by avoiding re-processing thousands of nested items.
    ...
    // ⚡ Bolt Optimization: Memoized flag count reduction to prevent re-execution on every render.
    // Why: Computing counts iterates over all notes in all competences of all groups.
    // Impact: Saves O(N) operations per render, improving UI responsiveness.
