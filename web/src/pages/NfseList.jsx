...
export default function NfseList() {
    ...
    // Apply all filters
    // ⚡ Bolt Optimization: Memoize the heavily nested filtering and mapping of groups and notes
    // This prevents expensive O(N*M) operations on every re-render, especially useful when
    // notes array is large and UI interactions (like expanding companies) trigger frequent updates.
    // Expected impact: Significant reduction in render time and UI blocking when interacting with the list.
    ...
    }, [groupedNfs, filterCompany, searchTerm, filterStatus]);

    // Count flags across all data for badge display
    // ⚡ Bolt Optimization: Memoize the calculation of flag counts
    // Prevents redundant iteration over all competences and notes on every re-render.
    // Expected impact: Faster re-renders, especially when interacting with inputs not related to groups.
